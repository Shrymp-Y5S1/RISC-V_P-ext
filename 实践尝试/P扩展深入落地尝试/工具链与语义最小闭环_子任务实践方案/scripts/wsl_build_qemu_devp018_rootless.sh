...
echo "[3/5] download rootless deps"
cd "$DEB_CACHE"
rm -f ./*.deb
apt download pkgconf pkgconf-bin libpkgconf3 libglib2.0-dev libglib2.0-dev-bin libpcre2-dev zlib1g-dev >/dev/null
for deb in ./*.deb; do
...
echo "[5/5] build qemu-riscv64"
make -C build -j"$(nproc)" qemu-riscv64

echo "QEMU source commit: $(git rev-parse --short HEAD)"
echo "QEMU binary: $SRC_DIR/build/qemu-riscv64"
